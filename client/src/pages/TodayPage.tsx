...
import { AiSessionTool } from '../components/AiSessionTool';
...
export function TodayPage() {
  ...
  const breakHistory = session?.breaks?.filter((b: SessionBreak) => b.resume_time) ?? [];
  const hasBreakHistory = isClockedIn && breakHistory.length > 0;
  const showSidePanel = hasBreakHistory || (!isClockedIn && !showHandoffPanel && lastHandoff);
    ...
    <div className="container">
      <div className={styles.topSplit}>
        <section className={styles.heroCard}>
          <div className={styles.heroTop}>
            <div>
              <p className={styles.pageTitle}>Today</p>
              <p className={styles.pageSubtitle}>Track your active session, pauses, and handoff notes in one tidy view.</p>
            </div>
            <ClockButton
          ...
          <aside className={styles.sideCard}>
            {hasBreakHistory && (
              <div className={styles.sidePanel}>
                <p className={styles.sectionTitle}>Break History</p>
                {breakHistory.map((b: SessionBreak) => (
            ...
            {!isClockedIn && !showHandoffPanel && lastHandoff && (
              <div className={styles.sidePanel}>
                <p className={styles.sectionTitle}>Previous Session</p>
      ...
      <div className={styles.mainSplit}>
        <section className={styles.sectionCard}>
          ...
          <CommitList commits={commits} onComment={handleComment} />
        </section>

        <section className={styles.sectionCard}>
          <p className={styles.sectionTitle}>Session Notes</p>
          <AiSessionTool
            commits={commits}
            currentNote={handoffNote}
            summary={session?.summary ?? lastCompleted?.summary}
            onApplyNote={setHandoffNote}
          />
          {showHandoffPanel ? (
            <div className={styles.handoffPanel}>
              <h3 className={styles.handoffTitle}>Session Handoff</h3>
            ...
            <DaySummary summary={lastCompleted.summary} />
          ) : (
            <p className={styles.empty}>No summary available yet.</p>
          )}
        </section>
      </div>
    </div>
